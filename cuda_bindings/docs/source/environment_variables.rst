...
- ``CUDA_PYTHON_COMPILER_LAUNCHER`` : A compiler launcher (e.g. ``sccache``) prepended to the compiler and linker selected by ``CUDA_PYTHON_TOOLCHAIN``. When set, the backend composes ``CC="<launcher> <cc>"`` (and likewise ``CXX``/``LDSHARED``) for the active toolchain, so CI can wrap the compiler for caching without naming it explicitly. No-op on Windows (MSVC is discovered via the MSVC environment, not ``CC``/``CXX``). Unset by default, in which case the backend defers to any externally-set ``CC``/``CXX``.
